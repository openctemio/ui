...
export const templateSourceKeys = {
  ...
  list: (filters?: TemplateSourceListFilters) => [...templateSourceKeys.lists(), filters] as const,
 ...
 */
export function useTemplateSources(filters?: TemplateSourceListFilters, config?: SWRConfiguration) {
 ...
 */
export function useTemplateSource(sourceId: string | null, config?: SWRConfiguration) {
  const { currentTenant } = useTenant()

  const key = currentTenant && sourceId ? templateSourceEndpoints.get(sourceId) : null
...
export function useUpdateTemplateSource(sourceId: string) {
  ...
  return useSWRMutation(
    currentTenant && sourceId ? templateSourceEndpoints.update(sourceId) : null,
...
export function useDeleteTemplateSource(sourceId: string) {
  ...
  return useSWRMutation(
    currentTenant && sourceId ? templateSourceEndpoints.delete(sourceId) : null,
    async (url: string) => {
      return del<void>(url)
    }
  )
...
export async function invalidateTemplateSourcesCache() {
  ...
  await mutate(
    (key) => typeof key === 'string' && key.includes('/api/v1/template-sources'),
    undefined,
    { revalidate: true }
  )
}
