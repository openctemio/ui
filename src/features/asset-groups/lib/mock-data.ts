...
export const getAssetGroupStats = () => ({
  ...
  byEnvironment: {
    production: mockAssetGroups.filter((g) => g.environment === 'production').length,
    staging: mockAssetGroups.filter((g) => g.environment === 'staging').length,
    development: mockAssetGroups.filter((g) => g.environment === 'development').length,
    testing: mockAssetGroups.filter((g) => g.environment === 'testing').length,
  },
  byCriticality: {
    critical: mockAssetGroups.filter((g) => g.criticality === 'critical').length,
  ...
  averageRiskScore: Math.round(
    mockAssetGroups.reduce((acc, g) => acc + g.riskScore, 0) / mockAssetGroups.length
  ),
})
