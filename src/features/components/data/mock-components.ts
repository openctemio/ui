...
const mockVulnerabilities: ComponentVulnerability[] = [
  {
    ...
    description:
      'A vulnerability in lodash allows attackers to inject properties via prototype pollution.',
...
export function getComponentStats(): ComponentStats {
  const ecosystemCounts = mockComponents.reduce(
    (acc, c) => {
  ...
  const typeCounts = mockComponents.reduce(
    (acc, c) => {
  ...
  const licenseRiskCounts = mockComponents.reduce(
    (acc, c) => {
  ...
  const licenseCategoryCounts = mockComponents.reduce(
    (acc, c) => {
  ...
  return {
    ...
    componentsInCisaKev: mockComponents.filter((c) => c.vulnerabilities.some((v) => v.inCisaKev))
      .length,
