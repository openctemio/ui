...
export function getRepositoryStats(
  repositories: RepositoryView[] | AssetWithRepository[] | undefined | null
) {
  const emptyStats = {
    total: 0,
  ...
  const withCriticalFindings = repositories.filter((r) => r.riskScore >= 80).length
  const avgRiskScore = repositories.reduce((sum, r) => sum + r.riskScore, 0) / total || 0
  const totalFindings = repositories.reduce((sum, r) => sum + r.findingCount, 0)
