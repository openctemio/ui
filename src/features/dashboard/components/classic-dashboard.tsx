...
/**
 * The previous ("classic") main dashboard, preserved in behaviour so it can be
 * switched back to from the CTEM action-first view. Self-contained: it fetches
 * its own data via useDashboardStats and owns its loading/error/empty states.
...
export function ClassicDashboard({ headerSwitcher }: { headerSwitcher?: ReactNode }) {
  ...
  return (
    <>
      ...
      {!isLoading && error && (
        <Card className={cn('mb-6', SEVERITY_BORDER_COLORS.medium)}>
          <CardContent className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              <AlertTriangle className={cn('h-5 w-5', SEVERITY_TEXT_COLORS.medium)} />
        ...
        !error && (
          <section className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <StatsCard
              ...
              change={stats.findings.total > 0 ? `${stats.findings.total} findings` : 'No findings'}
                    ...
                    <div className="space-y-4">
                      {stats.recentActivity.slice(0, 10).map((activity, index) => (
                        <ActivityItem
                          key={index}
                          icon={
                            <AlertTriangle className={cn('h-4 w-4', SEVERITY_TEXT_COLORS.medium)} />
                          }
