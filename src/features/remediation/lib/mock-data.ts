...
export const mockRemediationTasks: RemediationTask[] = [
  ...
  },
  {
    ...
    description: 'Add rate limiting middleware to prevent brute force attacks on API endpoints',
    status: 'open',
    priority: 'high',
    findingId: 'fnd-007',
  ...
  },
  {
    ...
    description: 'Configure firewall rules to restrict Redis port 6379 access from public internet',
    status: 'open',
    priority: 'medium',
    findingId: 'fnd-017',
  ...
  },
  {
    ...
    description: 'Update outdated npm packages with known CVEs in ecommerce frontend project',
    status: 'blocked',
    priority: 'medium',
    findingId: 'fnd-019',
...
export const getTaskStats = () => {
  const byStatus = {
    ...
    in_progress: mockRemediationTasks.filter((t) => t.status === 'in_progress').length,
    review: mockRemediationTasks.filter((t) => t.status === 'review').length,
    completed: mockRemediationTasks.filter((t) => t.status === 'completed').length,
