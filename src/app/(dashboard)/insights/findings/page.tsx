...
import { AlertTriangle, Shield, Clock, Bug } from 'lucide-react'
  ...
  return (
    <Main>
      <PageHeader
        ...
        description="Aggregated view of security findings across your organization"
        className="mb-6"
      />
      ...
      {isLoading ? (
        ...
        <>
          {/* Stats Row */}
          <section className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatsCard title="Total Findings" value={stats.findings.total} icon={Bug} />
            <StatsCard
              title="Critical + High"
              value={criticalHighCount}
              changeType={criticalHighCount > 0 ? 'negative' : 'positive'}
              change={
                stats.findings.total > 0
                  ? `${((criticalHighCount / stats.findings.total) * 100).toFixed(0)}% of total`
                  : undefined
              }
              icon={AlertTriangle}
            />
            <StatsCard
              ...
              change={stats.findings.overdue > 0 ? 'Needs attention' : 'On track'}
              icon={Clock}
            />
            <StatsCard
              title="Average CVSS"
          ...
          <section className="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Distribution of {stats.findings.total} findings across severity levels
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {/* Finding Trend Area Chart */}
          <section className="mb-6">
            <Card>
              <CardHeader>
                <CardTitle>Finding Trend</CardTitle>
                <CardDescription>Stacked severity counts over time</CardDescription>
              </CardHeader>
              <CardContent>
                {hasTrendData ? (
          ...
          <section>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Detailed count and percentage for each severity level
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
